...
public class AddressImg {
	public static BitmapDrawable getDrawable(String url) {
		...
		for (int i = 0; i < 2; i ++) {
			if (i > 0) Log.w(Settings.tag, "cgAddressImg.getDrawable: Failed to download data, retrying. Attempt #" + (i + 1));
			...
			try {
				...
				Log.i(Settings.tag, "[" + entity.getContentLength() + "B] Downloading address map " + url);

				if (bufferedEntity != null) imagePre = BitmapFactory.decodeStream(bufferedEntity.getContent(), null, null);
				if (imagePre != null) break;
			} catch (Exception e) {
				Log.e(Settings.tag, "cgAddressImg.getDrawable (downloading from web): " + e.toString());
			}
		}

		if (imagePre == null) {
			Log.d(Settings.tag, "cgAddressImg.getDrawable: Failed to obtain image");
