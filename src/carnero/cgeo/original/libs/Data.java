...
public class Data {
	...
	public String backupDatabase() {
		...
		try {
			...
			Log.w(Settings.tag, "Database wasn't backed up: " + e.toString());
		}
	...
	public boolean restoreDatabase() {
		...
		try {
			...
			Log.w(Settings.tag, "Database wasn't restored: " + e.toString());
		}
				...
				if (oldVersion > 0) {
					...
					if (oldVersion < 2) { // upgrade to 2
						try {
							...
							Log.e(Settings.tag, "Failed to upgrade to ver. 2: " + e.toString());
						}
					}
					*/
				}
	...
	public boolean saveSpoilers(String geocode, ArrayList<Spoiler> spoilers) {
		init();
		...
		try {
			...
			if (!spoilers.isEmpty()) {
				ContentValues values = new ContentValues();
				for (Spoiler oneSpoiler : spoilers) {
					values.clear();
					values.put("geocode", geocode);
					values.put("updated", System.currentTimeMillis());
	...
	public boolean saveLogs(String geocode, ArrayList<CacheLog> logs) {
		return saveLogs(geocode, logs, true);
	}

	public boolean saveLogs(String geocode, ArrayList<CacheLog> logs, boolean drop) {
		init();
	...
	public boolean saveInventory(String geocode, ArrayList<Trackable> trackables) {
		init();
		...
		try {
			...
			if (!trackables.isEmpty()) {
				ContentValues values = new ContentValues();
				for (Trackable oneTrackable : trackables) {
					values.clear();
					if (geocode != null) {
						values.put("geocode", geocode);
					}
	...
	public Cache loadCache(String geocode, String guid) {
		return loadCache(geocode, guid, false, true, false, false, false, false);
	}

	public Cache loadCache(String geocode, String guid, boolean loadA, boolean loadW, boolean loadS, boolean loadL, boolean loadI, boolean loadO) {
		...
		ArrayList<Cache> caches = loadCaches(geocodes, guids, null, null, null, null, loadA, loadW, loadS, loadL, loadI, loadO);
	...
	public ArrayList<Cache> loadCaches(Object[] geocodes, Object[] guids) {
		return loadCaches(geocodes, guids, null, null, null, null, false, true, false, false, false, false);
	}

	public ArrayList<Cache> loadCaches(Object[] geocodes, Object[] guids, boolean lite) {
		if (lite == true) {
			return loadCaches(geocodes, guids, null, null, null, null, false, true, false, false, false, false);
		} else {
	...
	public ArrayList<Cache> loadCaches(Object[] geocodes, Object[] guids, Long centerLat, Long centerLon, Long spanLat, Long spanLon, boolean loadA, boolean loadW, boolean loadS, boolean loadL, boolean loadI, boolean loadO) {
		init();
		...
		try {
			...
			cursor = databaseRO.query(
					...
					new String[]{
						...
						"description", "favourite_cnt", "disabled", "archived", "members", "found", "favourite", "inventorycoins", "inventorytags",
						"inventoryunknown"
					},
					where.toString(),
					null,
			...
			if (cursor != null) {
				...
				if (cursor.getCount() > 0) {
					...
					do {
						...
						if (loadW == true) {
							ArrayList<Waypoint> waypoints = loadWaypoints(cache.geocode);
						...
						if (loadS == true) {
							ArrayList<Spoiler> spoilers = loadSpoilers(cache.geocode);
						...
						if (loadL == true) {
							ArrayList<CacheLog> logs = loadLogs(cache.geocode);
						...
						if (loadI == true) {
							ArrayList<Trackable> inventory = loadInventory(cache.geocode);
	...
	public Waypoint loadWaypoint(Integer id) {
		init();
	...
	public ArrayList<Waypoint> loadWaypoints(String geocode) {
		init();
	...
	public ArrayList<Spoiler> loadSpoilers(String geocode) {
		...
		Cursor cursor = null;
		ArrayList<Spoiler> spoilers = new ArrayList<Spoiler>();
	...
	public ArrayList<CacheLog> loadLogs(String geocode) {
		init();
		...
		Cursor cursor = null;
		ArrayList<CacheLog> logs = new ArrayList<CacheLog>();
	...
	public Trackable loadTrackable(String geocode) {
		init();
	...
	public void clean(boolean more) {
		...
		try {
			...
			if (size > 0) {
				...
				databaseRW.execSQL("delete from " + dbTableCaches + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
				databaseRW.execSQL("delete from " + dbTableAttributes + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
				databaseRW.execSQL("delete from " + dbTableSpoilers + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
				databaseRW.execSQL("delete from " + dbTableLogs + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
				databaseRW.execSQL("delete from " + dbTableLogCount + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
				databaseRW.execSQL("delete from " + dbTableLogsOffline + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
				databaseRW.execSQL("delete from " + dbTableWaypoints + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ") and type <> \"own\"");
				databaseRW.execSQL("delete from " + dbTableTrackables + " where geocode in (" + Base.implode(", ", geocodes.toArray()) + ")");
	...
	public ArrayList<CachesList> getLists(Resources res) {
		init();

		Cursor cursor = null;
		ArrayList<CachesList> lists = new ArrayList<CachesList>();

		lists.add(new CachesList(true, 1, res.getString(R.string.list_inbox)));
		// lists.add(new CachesList(true, 2, res.getString(R.string.list_wpt)));
		...
		try {
			...
			if (cursor != null) {
				if (cursor.getCount() > 0) {
					...
					do {
						CachesList list = new CachesList(false);
			...
			Log.e(Settings.tag, "cgData.getLists: " + e.toString());
		}

		return lists;
	}

	public CachesList getList(int id, Resources res) {
		CachesList list = null;

		if (id == 1) {
			list = new CachesList(true, 1, res.getString(R.string.list_inbox));
		} else if (id == 2) {
			list = new CachesList(true, 2, res.getString(R.string.list_wpt));
		} else if (id >= 10) {
			init();
			...
			try {
				...
				Log.e(Settings.tag, "cgData.getList: " + e.toString());
			}
		}
