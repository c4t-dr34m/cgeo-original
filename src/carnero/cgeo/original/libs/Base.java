...
public class Base {
	...
	public int login() {
		...
		if (loginData != null && loginData.length() > 0) {
			if (checkLogin(loginData) == true) {
				Log.i(Settings.tag, "Already logged in Geocaching.com as " + loginStart.get("username"));
			...
			if (viewstate == null || viewstate.length() == 0) {
				Log.e(Settings.tag, "cgeoBase.login: Failed to find viewstate");
		...
		if (loginData != null && loginData.length() > 0) {
			if (checkLogin(loginData) == true) {
				Log.i(Settings.tag, "Successfully logged in Geocaching.com as " + login.get("username"));
				...
				if (loginData.indexOf("Your username/password combination does not match.") != -1) {
					Log.i(Settings.tag, "Failed to log in Geocaching.com as " + login.get("username") + " because of wrong username/password");

					return -6; // wrong login
				} else {
					Log.i(Settings.tag, "Failed to log in Geocaching.com as " + login.get("username") + " for some unknown reason");
	...
	public CacheWrap parseMapJSON(String url, String data) {
		...
		try {
				...
				if (count > 0 && extra.has("cc")) {
					...
					if (cachesData != null && cachesData.length() > 0) {
						...
						for (int i = 0; i < count; i++) {
							...
							final Cache cacheToAdd = new Cache();
			...
			Log.e(Settings.tag, "cgBase.parseMapJSON: " + e.toString());
		}
	...
	public Long parseGPX(App app, File file, int listId, Handler handler) {
		...
		try {
			GPXParser GPXparser = new GPXParser(app, this, listId, search);
		...
		Log.i(Settings.tag, "Caches found in .gpx file: " + app.getCount(searchId));
	...
	public String getHumanSpeed(float speed) {
		...
		if (this.settings.units == Settings.unitsImperial) {
			kph *= kmInMiles;
			unit = "mph";
		}
	...
	public Long searchByNextPage(SearchThread thread, Long searchId, int reason, boolean showCaptcha) {
		final String viewstate = app.getViewstate(searchId);
		final String viewstate1 = app.getViewstate1(searchId);
		CacheWrap caches = new CacheWrap();
	...
	public Long searchByOffline(HashMap<String, Object> parameters) {
		...
		final Search search = app.getBatchOfStoredCaches(true, latitude, longitude, cachetype, list);
	...
	public Long searchByHistory(HashMap<String, Object> parameters) {
		...
		final Search search = app.getHistoryOfCaches(true, cachetype);
		search.totalCnt = app.getAllHistoricCachesCount(true, cachetype);

		return search.getCurrentId();
	}

	public Long searchByCoords(SearchThread thread, HashMap<String, String> parameters, int reason, boolean showCaptcha) {
		final Search search = new Search();
		...
		if (latitude == null || latitude.length() == 0) {
			Log.e(Settings.tag, "cgeoBase.searchByCoords: No latitude given");
			return null;
		}

		if (longitude == null || longitude.length() == 0) {
			Log.e(Settings.tag, "cgeoBase.searchByCoords: No longitude given");
			return null;
		}
	...
	public Long searchByKeyword(SearchThread thread, HashMap<String, String> parameters, int reason, boolean showCaptcha) {
		final Search search = new Search();
	...
	public Long searchByUsername(SearchThread thread, HashMap<String, String> parameters, int reason, boolean showCaptcha) {
		final Search search = new Search();
	...
	public Long searchByOwner(SearchThread thread, HashMap<String, String> parameters, int reason, boolean showCaptcha) {
		final Search search = new Search();
		final String userName = parameters.get("username");
		CacheWrap caches = new CacheWrap();
		String cacheType = parameters.get("cachetype");
	...
	public Long searchByViewport(HashMap<String, String> parameters, int reason) {
		...
		if (latMin == null || latMin.length() == 0 || latMax == null || latMax.length() == 0 || lonMin == null || lonMin.length() == 0 || lonMax == null || lonMax.length() == 0) {
			Log.e(Settings.tag, "cgeoBase.searchByViewport: Not enough parameters to recognize viewport");
			return null;
		}
	...
	public ArrayList<User> getGeocachersInViewport(String username, Double latMin, Double latMax, Double lonMin, Double lonMax) {
	...
	public Trackable searchTrackable(HashMap<String, String> parameters) {
		...
		if ((geocode == null || geocode.length() == 0) && (guid == null || guid.length() == 0) && (id == null || id.length() == 0)) {
			Log.e(Settings.tag, "cgeoBase.searchTrackable: No geocode nor guid nor id given");
			return null;
		}
	...
	public int postLog(App app, String geocode, String cacheid, String viewstate, String viewstate1, int logType, int year, int month, int day, String log, ArrayList<TrackableLog> trackables) {
		...
		if (trackables != null) {
			Log.i(Settings.tag, "Trying to post log for cache #" + cacheid + " - action: " + logType + "; date: " + year + "." + month + "." + day + ", log: " + log + "; trackables: " + trackables.size());
		} else {
			Log.i(Settings.tag, "Trying to post log for cache #" + cacheid + " - action: " + logType + "; date: " + year + "." + month + "." + day + ", log: " + log + "; trackables: 0");
		}
	...
	public static void trustAllHosts() {
		...
		try {
			...
			Log.e(Settings.tag, "cgBase.trustAllHosts: " + e.toString());
		}
	}
		...
		try {
			...
			for (int i = 0; i < translationCnt; i ++) {
				...
				if (toTranslate == false) {
					...
					Log.i(Settings.tag, "Translating #" + i + ": " + language + ">" + target);
					translated.add(jsonTranslation.getString("translatedText"));
				}
			}
		} catch (Exception e) {
			Log.w(Settings.tag, "Base.translate: " + e.toString());
		}
	...
	*/
	...
	public static String implode(String delim, Object[] array) {
		...
		try {
			...
			Log.e(Settings.tag, "cgeoBase.implode: " + e.toString());
		}
		return out;
	}
	...
	public String requestViewstate(boolean secure, String host, String path, String method, HashMap<String, String> params, boolean xContentType, boolean my) {
		final Response response = request(secure, host, path, method, params, xContentType, my, false);
	...
	public String requestLogged(boolean secure, String host, String path, String method, HashMap<String, String> params, boolean xContentType, boolean my, boolean addF) {
		Response response = request(secure, host, path, method, params, xContentType, my, addF);
	...
	public Response request(boolean secure, String host, String path, String method, HashMap<String, String> params, boolean xContentType, boolean my, boolean addF) {
		// prepare parameters
	...
	public Response request(boolean secure, String host, String path, String method, HashMap<String, String> params, int requestId, boolean xContentType, boolean my, boolean addF) {
		// prepare parameters
		final String paramsDone = prepareParameters(params, my, addF);
	...
	public Response request(boolean secure, String host, String path, String method, String params, int requestId, Boolean xContentType) {
		URL u = null;
		int httpCode = -1;
		...
		for (int i = 0; i < 5; i++) {
			...
			try {
				...
				if (buffer != null && connection != null) {
					Log.i(Settings.tag + "|" + requestId, "[" + method + " " + (int)(params.length() / 1024) +  "k | " + httpCode + " | " + (int)(buffer.length() / 1024) + "k] Downloaded " + scheme + host + path + "?" + paramsLog);
				} else {
					Log.i(Settings.tag + "|" + requestId, "[" + method + " | " + httpCode + "] Failed to download " + scheme + host + path + "?" + paramsLog);
				}
	...
	public String requestJSONgc(String host, String path, String params) {
		...
		for (int i = 0; i < 3; i++) {
			...
			try {
				...
				final String paramsLog = params.replaceAll(passMatch, "password=***");
				Log.i(Settings.tag + " | JSON", "[POST " + (int)(params.length() / 1024) + "k | " + httpCode + " | " + (int)(buffer.length() / 1024) + "k] Downloaded " + "http://" + host + path + "?" + paramsLog);
		...
		for (int i = 0; i < 3; i++) {
			...
			try {
				try {
					...
					Log.e(Settings.tag, "cgeoBase.requestJSON.IOException: " + httpCode + ": " + connection.getResponseMessage() + " ~ " +  e.toString());
				}
			} catch (Exception e) {
	...
	public boolean runExternalMap(int application, Activity activity, Resources res, Warning warning, GoogleAnalyticsTracker tracker, Double latitude, Double longitude) {
		// waypoint
		return runExternalMap(application, activity, res, warning, tracker, null, null, latitude, longitude);
	}

	public boolean runExternalMap(int application, Activity activity, Resources res, Warning warning, GoogleAnalyticsTracker tracker, Waypoint waypoint) {
		// waypoint
		return runExternalMap(application, activity, res, warning, tracker, null, waypoint, null, null);
	}

	public boolean runExternalMap(int application, Activity activity, Resources res, Warning warning, GoogleAnalyticsTracker tracker, Cache cache) {
		// cache
		return runExternalMap(application, activity, res, warning, tracker, cache, null, null, null);
	}

	public boolean runExternalMap(int application, Activity activity, Resources res, Warning warning, GoogleAnalyticsTracker tracker, Cache cache, Waypoint waypoint, Double latitude, Double longitude) {
	...
	public boolean runNavigation(Activity activity, Resources res, Settings settings, Warning warning, GoogleAnalyticsTracker tracker, Double latitude, Double longitude) {
		return runNavigation(activity, res, settings, warning, tracker, latitude, longitude, null, null);
	}

	public boolean runNavigation(Activity activity, Resources res, Settings settings, Warning warning, GoogleAnalyticsTracker tracker, Double latitude, Double longitude, Double latitudeNow, Double longitudeNow) {
		if (activity == null) {
			return false;
		}
		if (settings == null) {
		...
		public void run() {
			try {
				...
				if (tracker == null && context != null) {
					...
					tracker.start(Settings.analytics, context);
				}

				tracker.trackPageView(page);
				tracker.dispatch();

				Log.i(Settings.tag, "Logged use of " + page);
